...
class Factor_Analysis(object):  # Adjust SAVE_PATH if required
    SAVE_PATH = r"result"  # Directory used to store outputs
    BENCHMARK_PATH = "choice_a.xlsx" # Path to the benchmark index file
    POOL = 'A Share'

    def __init__(self, factor_df: pd.DataFrame, factor_name: Optional[str] = None, codes: Optional[List[str]] = None,
                 start: Optional[str] = None,
                 end: Optional[str] = None, freq: int = 1):
        self.SAVE_PATH = os.path.join(self.SAVE_PATH, str(freq))
        self.profit = None
        self.hierarchical_constitute = None  # Holdings purchased after each holding window
        self.freq = freq
        self.codes = codes
        self.Kline = Kline(codes=codes, start=start, end=end)
        ...
        self.trading_day = self.trading_day[self.trading_day.isin(factor_df.index)][
                           :-self.freq]  # Drop the last N days because forward returns consume them
        ...
        self.r_next = self.align(self.get_r_next(), dates=self.trading_day.tolist(),
                                 codes=self.std_factor_df.columns.tolist())  # Align with available factor symbols
    ...
    @staticmethod
    def three_sigma(cs_factor: pd.Series) -> pd.Series:  # Cross-sectional outlier clipping
    ...
    def align(original_factor: pd.DataFrame, dates: Optional[List[pd.Timestamp]] = None,
              codes: Optional[List[str]] = None) -> pd.DataFrame:  # Restrict to the trading calendar and pool
        if codes:
            if dates:
    ...
    def standard(self, cs_factor: pd.Series, type: Optional[str] = 'Minmax') -> pd.Series:  # Cross-sectional standardisation
    ...
    def preprocess(self) -> pd.DataFrame:  # Apply cross-sectional clipping and scaling
    ...
    def Stratify_r(self, date: pd.Timestamp) -> Tuple:  # Return next-period returns and bucket constituents
        print(date)
    ...
    def Period_NV_Backtest(self) -> pd.DataFrame:  # Net value trajectory for each bucket

        period_factor_NV = pd.DataFrame()
        hierarchical_constitute = pd.DataFrame()
        for date in self.trading_day:
            factor_r, daily_constitute = self.Stratify_r(date)  # Forward-period return
        ...
        return period_factor_NV  # Time-series returns for each factor bucket

    def Backtest(self):  # Compute the aggregated single-factor backtest
    ...
    def bar_plot(self, figure_name: str = 'turnover'):
        ...
        # plt.savefig(self.SAVE_PATH+rf'\Turnover distribution for {self.name} starting {dt.strftime(self.trading_day[0],'%Y%m%d')}.png')
...
if __name__ == '__main__':
    alphanet_prediction = pd.read_csv('result/model_prediction1.csv').dropna()  # Replace with actual file path

    alphanet_prediction = pd.pivot_table(index='trade_date', columns='st_code', values='prediction', data=alphanet_prediction)
    alphanet_prediction.index = pd.to_datetime(alphanet_prediction.index.astype(str))
    date_range = Kline.get_trading_days(alphanet_prediction.index[0].strftime('%Y%m%d %H:%M:%S'),alphanet_prediction.index[-1].strftime('%Y%m%d %H:%M:%S'))
    alphanet_df = pd.DataFrame(index=pd.to_datetime(date_range),columns=alphanet_prediction.columns)
    alphanet_df.loc[alphanet_prediction.index,:] = alphanet_prediction
    alphanet_df = alphanet_df.ffill()
    backtester = Factor_Analysis(alphanet_df, 'alphanet', start='20230101', freq=5)  # Holding period set to 5
