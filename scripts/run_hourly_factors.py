...
def register_factor(registry: FactorRegistry, spec: FactorSpec) -> None:
    ...
    if name in registry._factors:  # type: ignore[attr-defined]
        LOGGER.info("Removing existing registration for '%s'", name)
        registry.delete_factor(name, reason="Re-register hourly factor")
